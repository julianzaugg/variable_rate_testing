"""
Package for variable rate testing of ancestral sequence reconstruction tools.

Code provided here prepares input data for the tools - FASTML, PAML and GRASP.
Specifically, given an input multiple sequence alignment and a tree:

    1. Identify from the alignment user-defined subsets of sequences
    2. For each subset, copy original tree and prune to only include the sequences in the subset while maintaining
        a binary tree structure.
    3. Identify those positions that are gaps so they can be ignored and reinserted later.


After file preparation, the user can generate a shell script with provided parameters. This shell script
will initiate the individual run jobs (OR JUST NOHUP IN PYTHON?). The following steps are taken:

    4. For each subset alignment and corresponding tree, infer the ancestral sequences using the three tools


Finally, this package provide analysis tools for the output for each of the three tools:

    5. Compare each ancestor from the tree by calculating their evolutionary distance. Specifically
        using the program PROTDIST from the PHYLIP package. Since the tree was pruned, this will mean
        we only compare the root ancestor.

    6. Output result tables and perform basic analysis.

input data parameters
    - number of repeats
    - number of groups for each repeat

input data
    number_of_groups
        repeat
            logfile (locations where gaps belong)
            seqs.fasta
            tree.newick
            seqs.phylip
            tree_phylipnames.newick
...
PAML : change alpha to 0.5 for variable/ 0.0 for static
